...
The `ALTER TABLE` statement allows for structural changes like:

- Adding new columns
- Modifying existing ones
- Deleting columns
- Renaming columns within a table

To alter/modify the table, use the following syntax:
...
* `MODIFY`: Used to change the datatype or definition of an existing column.
...
You can rename an entire table using the `RENAME TO` clause. This changes the name of the table while preserving its structure and data.
...
To add a new column to an existing table, you can use the `ADD` clause. Here’s an example where we add a column named `email` to the `employee` table:
...
We can remove an existing column from a table using the `DROP COLUMN` clause. Here’s an example where we remove the `phone_number` column from the `employee` table:
...
To change the data type or size of an existing column, you can use the `MODIFY` clause. For instance, if you want to change the data type of the `salary` column from `INT` to `DECIMAL`, you can use:
...
The SQL `ALTER TABLE` statement is an essential tool for modifying the structure of an existing table. Whether you need to add, delete, or modify columns, or even rename the table or columns, `ALTER TABLE` provides the flexibility needed to manage your database schema efficiently.
